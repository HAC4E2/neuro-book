# Route B：TTP（text-to-picture）分镜预设迁移与 Agent 化章节插图设计

日期：2026-07-17
状态：方案已确认，待实现（implementation-ready design）
修订：2026-07-17 采用 A 方案；加入选区单图、跨章节并发、结构化按钮发布、唯一 NovelAI Provider/API、15 秒全局 lane、TTP Tag Pattern 伴生工件、Danbooru 官方 3K+ 分层本地索引、彻底排除 TTP `tagData/`，以及“Director LLM 归设置模型配置、NovelAI 参数归文生图分页”的双配置边界
...
本文仅在“TTP 分镜预设导入、章节语义规划与正文专用 LLM 链路”范围内，部分取代：
...
1. 采用 Route B：迁移 TTP 的能力和数据语义，不嵌入或复刻 TTP UI/runtime。
...
4. 使用一个固定 Skill `novel-import-ttp-storyboard-preset` 转换上传 JSON；每份 JSON 生成一份数据型 Markdown Storyboard Preset candidate 和一份 Tag Pattern companion candidate，不生成动态 Skill。
...
8. Danbooru 官方原子 Tag 词库是本地服务端索引；TTP 分镜预设中的场景组合 Tag 迁移为独立 Markdown Tag Pattern Set。二者不进入 Skill 或 Agent 整包上下文，也不能互相摊平；Pattern 不是 TTP `tagData/` 的副本。
9. 从 TTP 导出的标准 SillyTavern 角色卡/PNG，以及 Context preset JSON 中可确定识别的结构化角色和服装字段，可以导入 NeuroBook；它们必须进入对应 Project 的 `lorebook/character/**`，不能进入全局分镜预设。未知 TTP 私有角色格式只报告，不猜测兼容。
...
16. 每份 TTP JSON 同时生成一份主 Storyboard Preset candidate 和一份同 `presetId/importId` 的 Tag Pattern companion candidate；两者作为一个不可变版本包发布，selector 最后切换。Project 对二者分别按稳定 ID 做增量覆盖。
17. 原子 Tag 库的唯一 canonical 来源是 Danbooru 官方 API：用户触发后完整同步 `post_count >= 3000` 的 Tag，并收集指向这些 Tag 的有效 Alias 与相关 Implication，构建 Workspace Root 本地索引。彻底放弃 TTP `tagData/`：不下载、不导入、不解密、不转换、不缓存，也不分发其数据或衍生词库。
...
- 将 TTP Context JSON 中可复用的分镜、选景、构图、画幅、连续性和 Tag 策略迁移成可读、可审查、可版本化的 Markdown。
...
- 支持 TTP 角色资产迁入现有角色 Markdown 体系。
- 保留 TTP 场景组合中有价值的 Tag 共现、构图、光照和动作经验，同时继续用原子 Tag 索引做标准化与兜底。
...
- 不原样执行完整 TTP Context Prompt，不继承其中 `system/user/assistant` 权限。
- 不承诺相同模型下逐字、逐镜头或比特级复现 TTP 输出。
- 不把用户 JSON 变成 Skill、Profile、脚本或工具权限。
- 不把 TTP Context preset 当成 NovelAI sampler/模型参数 Recipe。
...
- 不提供 TTP `tagData/` 的下载器、目录选择器、解密器、source adapter、enrichment 或兼容开关；该数据源不进入当前路线和未来预留接口。
...
因此，产品文案应写“导入并迁移 TTP 分镜预设”，不能写“100% 原样运行 TTP Context”。
...
macros:
  ...
  unresolved: []
rules:
  - ruleId: ttp.089bf996.shot-selection.even-distribution
  ...
  - ruleId: ttp.00be1a4a.composition.single-instant
...
└─ novel-import-ttp-storyboard-preset/
   └─ SKILL.md
```
...
`semanticSlot` 必须来自每种 rule kind 注册的稳定槽位，不是自由字符串。建议 final ID 形态为 `ttp.<entryIdHash>.<ruleKind>.<semanticSlot>`；显式 source entry ID 未变且语义仍落在同一槽位时，重转换继续得到同一 ruleId。缺少显式 ID 的 entry 一旦内容变化可能获得新 identity，report 必须说明相关 Project overlay 会 stale，不能假装稳定。
...
- 首版支持两类有明确输入合同的来源：从 TTP 导出的标准 SillyTavern character card/PNG 复用现有 `novel-import-silly-tavern-card` 工作流；上传的 Context preset JSON 中发现的结构化角色列表/字段进入 import report，并提供“作为 Project 角色继续导入”的入口。
- 不在缺少真实样例和 schema 的情况下承诺 TTP 私有 character group/export 兼容 adapter。若用户提供另一种专有导出格式，必须先单独 inspect、冻结 fixture 与字段映射，再作为新的确定性 parser 立项；不能用 Agent 猜格式。
...
这使产品同时满足：导入 TTP 分镜预设、导入 TTP 已有角色、并保持 NeuroBook 的 Project Markdown 结构。
...
不提交图片数据或上游 Tag 数据快照。NeuroBook 仓库和发行包不内置 Danbooru/TTP Tag 数据；官方 Danbooru 索引只由用户在产品内主动触发同步并在本地构建。
...
字段所有权固定由 Danbooru 官方记录决定：canonical name、category、post count、deprecated、官方 alias 与 implication 均保留 source record 和 fetch provenance。用户上传的 TTP 分镜预设只能产生 Storyboard/Tag Pattern proposal；其中出现的 Tag 必须经过官方索引 Resolver，形成 canonical/replacement/passthrough 决策，绝不能反向写入原子索引、创建“TTP Tag”或覆盖官方事实。

本路线明确禁止实现 TTP `tagData/` 的任何运行时能力，包括下载、用户本地目录导入、Base64/AES 解密、翻译/分类 enrichment、兼容 decoder、source manifest 和导出。若未来重新评估，必须作为新的产品与许可决策重新立项；本文不保留 adapter、接口或测试占位。
...
TTP 分镜预设中的场景 Pattern 同样不具备内容分级权威：导入器只能保留来源和风险提示，Pattern 内每个 Tag 在批准与编译时仍受本地 tag-policy 和官方索引校验，不能因为它来自预设样例就自动判为安全。
...
原子索引回答“某个 Tag 是什么、是否存在、有哪些 alias、能否用于当前模型”；它不表达“雨夜小巷通常应如何同时组织场景、镜头、光照、动作和排除项”。TTP 样例中大量场景组合的价值正是这种共现经验。如果把它们拆成无关系的单词，组合知识会丢失；如果把整段组合串当成一个伪 Tag，alias、冲突、权重和 Provider 校验又会失效。
...
patterns:
  - patternId: ttp.a12f09.scene-recipe.rainy-night-alley
...
shots:
  - shotId: shot_sb01_01
    ...
    continuity:
      timeOfDay: dawn
      palette: silver-blue
    tagPatternRefs: [ttp.a12f09.scene-recipe.harbor-dawn]
    tagResolutions:
      tr-fleet:
        schemaVersion: nbook.semantic-tag-resolution/v1
        kind: canonical
...
- `novel-import-ttp-storyboard-preset`：从 upload 导入 pending Storyboard/Tag Pattern companion pair，预览分类与 Recipe proposal，批准完整 package。
...
Tag 索引自动检测；首次安装必须由用户点击“同步 Danbooru 3K+ Tag 库”。UI 显示固定阈值、官方来源、版本/watermark、预计记录数/体积、terms、分页进度、四层记录数、checksum、完整度和边界 reconciliation 结果。下载、建库和校验在后台可恢复执行，一次安装供 Workspace Root 全局复用；不因打开页面而静默抓取全站。产品中不出现 TTP `tagData/` 下载、导入、解密或 enrichment 入口。
...
- `app/utils/text-to-image-st-ttp-import.ts`：把 parser/ID/转换语义移到 shared/server，前端只做 DTO/UI。
...
- 新增确定性 TTP inspect/classifier/converter/import journal 和固定 Skill；每个 JSON 产出同 `presetId/importId` 的 companion candidates，并把 `style_quality` 分流为未激活 Recipe proposal。
...
- 实现统一角色/服装 Project migration：先扫描已有 `image-tags.md/outfits/*.md`，再接入从 TTP 导出的标准 SillyTavern card/PNG 与 Context preset 结构化角色字段；所有自由 Tag 都先形成 `PendingTagAtom` proposal，经 generic NovelAI scope Resolver、逐项确认、typed snapshot codec、tracked-write journal 与中断恢复后原子升级。确定性 identity 匹配并保留用户字段；未知 TTP 私有导出格式 report-only，不猜 parser，也不依赖 `tagData/`。
...
- `server/text-to-image/ttp-storyboard-converter.ts`
...
- `assets/workspace/.nbook/agent/profiles/builtin/illustration.director.profile.tsx`
- `assets/workspace/.nbook/agent/skills/novel-import-ttp-storyboard-preset/SKILL.md`
...
- 从 TTP 导出的标准 SillyTavern card/PNG 走既有通用 importer；Context preset 的结构化角色/服装字段只形成 Project proposal，不写全局 preset；
...
- 未知 TTP 私有角色导出格式 report-only；角色迁移 fixture 和运行路径不读取、不下载 TTP `tagData/`；
...
- 负向范围测试：API、UI、Profile、Skill、source registry 与构建产物中均不存在 TTP `tagData/` 下载、目录导入、解密、enrichment 或 adapter；TTP 分镜预设抽出的 Pattern 只能保存 Resolver 产出的 canonical/replacement/passthrough 节点，不能向原子库新增记录。
...
| 用户目标 | 完成证据 |
| --- | --- |
| 1. 使用 TTP 文生图/分镜预设 | 用户样例经固定 Skill 生成 Storyboard + Tag Pattern 两份 pending Markdown；report 解释兼容范围；批准后 Director 的选景、Pattern refs 和最终编译 fixture 明确受到规则/组合影响 |
| 2. 导入 TTP 已生成角色 | 从 TTP 导出的标准 SillyTavern card/PNG，或 Context preset JSON 中可识别的结构化角色字段，可与 Project 已有角色视觉 Tag 进入同一 migration proposal；确认后以 typed resolution snapshots 写入 Project `index.md + image-tags.md + outfits/*.md`，已有用户字段不被静默覆盖；无 Tag index 时保持 pending，未知私有导出格式不做猜测兼容 |
...
| 放弃 TTP Tag 数据包 | 产品与代码中不存在 `tagData/` 下载、导入、解密、enrichment、adapter 或再分发路径；分镜 Pattern 与角色资产导入都不依赖该数据包 |
...
- TTP Context JSON 能经固定 Skill 产出可审查的 Storyboard Preset + Tag Pattern companion Markdown，而不是动态 Skill 或原 Prompt 注入；
...
- TTP `tagData/` 在 API、UI、Profile、Skill、source registry、测试 fixture 和发行物中均无下载、导入、解密、enrichment 或 adapter；TTP 分镜 Pattern 与用户自有角色资产导入不依赖该数据包；
