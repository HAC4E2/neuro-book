...
kind: architecture
status: planned
capability: architecture.monorepo-boundaries
owners:
  - architecture
---
...
目标应用最终位于 `packages/neuro-book`。本轮只冻结边界和迁移步骤，不搬移 `app/`、`server/`、`shared/`、`assets/`、`world-engine/`、`scripts/` 或 `desktop/`，也不批量改写现有 `nbook/*` 导入。

## 当前布局与目标布局

| 边界 | 当前真相源 | 当前入口或消费方 | 目标状态 |
| --- | --- | --- | --- |
| 根应用 | 根 `package.json`、`app/`、`server/`、`shared/`、`nuxt.config.ts` | `bun run dev`、Nuxt、根 Vitest | 迁入 `packages/neuro-book`；根只保留 workspace 编排 |
| Source Dev | `scripts/cli/source-dev.ts` 与 `scripts/cli/` | `package.json#dev` | 继续作为宿主适配层；迁移后指向应用包的公开入口 |
| Agent Runtime | `server/agent/`、相关 `shared/dto/` 和稳定 `reference/agent/` | Agent API、Harness、Profile runtime | 先保持逻辑 Module；只有 Interface 和测试闭合后才迁移物理路径 |
| Project / Workspace | `server/workspace-files/`、Project SQLite、Workspace 文件协议 | Project API、文件工具、Workspace CLI | 由 Project Workspace Module 持有数据和授权；不由 UI 或脚本直接拼路径 |
| World Engine | `world-engine/` 与 `reference/world-engine/` | Plot、Agent tools、写作流程 | 保持独立领域 Module；Product runtime 通过显式 runtime island 消费 |
| Product Runtime / Release | `server/runtime/`、`scripts/build/`、`scripts/deploy/`、`scripts/release/` | Product、Portable、Container、Release | 共享验证和发布入口保持单一 owner；脚本只做宿主适配 |
| Manager | `packages/neuro-book-manager/` | `@notnotype/neuro-book-manager`、`neuro-book` bin | 独立包；不得反向依赖 Nuxt 页面或根应用特例 |
...
当前根 `tsconfig.json` 的 `nbook/*` alias 指向仓库根；它是本轮应用源码的绝对导入约定，不是已经发布的包 Interface。迁移前不得把 alias 误当成目标目录已经存在。
...
6. **隔离运行数据**：测试、验收、cache 和 scratch 使用 `NBOOK_AGENT_TEMP_ROOT` 及受控子目录；任何 fixture 不得写入仓库 `.agent/tmp/`、`.worktree/` 或目标包源码树。
...
- 不把主应用物理移动到 `packages/neuro-book`。
- 不把根 `package.json` 立即改成只含 workspace 编排的空壳。
- 不批量重写约 4,400+ 个 `nbook/*` 导入。
...
只有以下条件同时成立，才允许把主应用迁入 `packages/neuro-book`：

- 根 workspace orchestrator、`packages/neuro-book` 和所有现存包的依赖图无反向环；
- Source Dev、Nuxt、Vitest、Product build、Manager 和 Desktop Contract 都从明确入口工作；
- `nbook/*` 已由目标包内 alias 或公开 exports 承接，且不存在 root-only fallback；
...
相关长期边界见根 [AGENTS.md](../../../AGENTS.md)、[packages/AGENTS.md](../../../packages/AGENTS.md)、[ADR 0015](../../adr/0015-architecture-boundaries-and-deferred-structure.md) 和 [ADR 0009](../../adr/0009-product-runtime-image-generation.md)。
