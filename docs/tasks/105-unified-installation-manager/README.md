...
- workflow `30751019906` 的 Agent State Root smoke 在约 3.3 秒内完成两阶段断言并输出 `{ok:true}`，但父 Bun 进程因全局 Project Session Service/ProjectLifecycle 未关闭而继续存活约 15 分钟，最终人工取消；没有进入任何构建、资产或 OCI fan-out。脚本现使用 `closeAllProjects()` 完成全局生命周期所有权，并给该 workflow step 设置 10 分钟上限；Windows 真实移动/恢复 smoke 6.4 秒自然退出。该 Draft 继续保留取消审计，下一 Candidate 不复用其身份。
- 下一 Draft `v0.9.0-canary.20260802.142014Z.fa92005b`（release ID `363806130`）的 workflow [`30751890962`](https://github.com/notnotype/neuro-book/actions/runs/30751890962) 已证明生成前置和 State Root 生命周期修复；最后的 release assets Vitest 因复用全仓 Agent/Nuxt setup，在 clean hoisted 环境收集测试前遭遇无关 Zod export 错误。资产/checksum 合同现使用无全局 fixture 的专用 Vitest config，2 files / 22 tests 通过；旧 Draft 保留失败审计，仍未产生 Product、Portable 或 OCI fan-out。

### 2026-08-02：第四次 Candidate 的 Manager 边界
...
- 本地验证为Manager 36 files passed / 1 skipped、242 passed / 3 skipped，Manager typecheck与5-file pack通过；Release合同3 files / 26 tests、scripts typecheck和docs build全绿。Manager `.43`已由workflow [`30769613583`](https://github.com/notnotype/neuro-book/actions/runs/30769613583)完成Trusted Publishing；npm `gitHead=6d62e57387820b501cdc51022b13bb8607c0d137`，公开tarball SHA-1为`43cc2e38aa754383bb94f49066039f050e28b816`，registry signature、SLSA provenance、真实`bunx`与`manager:verify-public`均通过。下一步创建全新0.9 Candidate；Candidate 17保留10资产Draft审计，未公开Release或激活正式OCI tag。

### 2026-08-03：第十八次 Candidate 的两阶段 Compose volume 布局

- Draft `v0.9.0-canary.20260802.221657Z.3e2b7d33`（release ID `363907966`、revision `c2bc42e2dfe2980ec1632fcd3f2d636d57fe022a`）的 workflow [`30769774882`](https://github.com/notnotype/neuro-book/actions/runs/30769774882) 已有17个job成功：五平台Product、Windows Portable、双OCI、assemble、Linux最终验证、Windows Runtime/浏览器/鉴权重启/shutdown/lease/两种自卸载、Draft 10个payload实字节复核和0.8.6完整data复用全部通过。Candidate 17的候选Compose ENOENT已关闭。
...
- 回归测试使用真实候选Compose、`migration-plan-state`和`migration-plan-cache`形状，固定候选`./...`与激活后`../...`两组路径；focused Docker/Updater为33/33，Manager全量242 passed / 3 skipped，typecheck通过。Release合同与Manager全量并行时一个5秒Source archive测试因资源竞争耗时5.685秒，隔离重跑恢复26/26；不调大timeout。修复改变Manager bundle，需发布并验证`.44`后创建全新Candidate；Candidate 18保持10资产Draft、未公开且未激活正式OCI tag。
