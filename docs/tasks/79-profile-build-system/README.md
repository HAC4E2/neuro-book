...
- [x] 2026-08-02 发行门禁补强：`ProfileBuildCoordinator.flush()`可等待当前debounce窗口、活动pump与stale重排队全部排空，并在编译失败或 freshness/Registry 恢复失败时拒绝；`closing`终态阻止teardown后重新enqueue/pump。异步 `enqueue()` / `bootSweep()` 扫描作为 producer 登记，`dispose()` 会先封闭入口，再等待 producer 与活动pump；Harness 也把 startup reconcile 与 watcher startup 纳入后台任务。Catalog先关闭watcher producer再关闭Coordinator并聚合错误。Coordinator/Catalog及直接链路聚焦回归已覆盖真实debounce与 in-flight Catalog snapshot；普通HTTP保存仍只`enqueue`，没有把后台编译改成同步请求。
