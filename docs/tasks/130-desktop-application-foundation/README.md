...
- 自卸载实测发现公开 Manager `.38` 的 detached Host 没有运行，不能把同步 Host 脚本单测冒充完整生命周期证据。调度边界已改为短命 PowerShell launcher + 独立 Host 并新增真实行为测试；`.39` 公开和使用 `.39` 重组 Portable 后，必须重跑默认保留 `data/` 与 `--delete-data` 两条链。
