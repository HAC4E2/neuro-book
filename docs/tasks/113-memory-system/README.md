...
3. **跑 2 未完成**：摄入正常（338 条 / 23 次 LLM / 290s），答题阶段遇 mimo 网关整体下线（503「没有可用的内网节点」）中止。已挂守候任务，恢复后自动补跑。
...
- [ ] 遗留优化项（正式实现阶段做，非 spike 债）：**主体识别只有字面包含一条路（ADR 0005 未决 #5，2026-08-01 实验证明是当前投入产出比最高的缺口，同时解 p037 与 p018 反向专名题）**、状态检索命中率（topic 归一化 / 当前状态清单注入）、revision 探针扩到 ≥24 道再谈优化。
- [ ] 把查询计划接进 bench 评测路径（当前零覆盖）；接法需保留 `search()` 对照以维持与历史跑次的可比性，建议加 `--planner heuristic|llm|off` 开关、off 为默认。
- [ ] 补第一轮回归的第二跑取均值（阻塞于 mimo 网关恢复，守候任务已挂）。
