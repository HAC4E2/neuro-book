...
### 第一轮收口（2026-08-01 起，进行中）

用户拍板两个分叉后开工，计划见 [PLAN-round1-closeout.md](./PLAN-round1-closeout.md)：

- **describedAs 接入方式 = `search()` 内兜底优先**（不是补规划器 + `--planner`）。关键调研发现：**两个规划器都不产出 `describedAs`**，所以「把查询计划接进评测」这一步单独做完，新能力仍然不会被触发；而 `search()` 里主体锚定只有一个入口（`src/index.ts:365`），`queryVec` 在上一行已算好——在那里兜底是**零额外网络调用**的。
- **收口边界 = nb-memory 库收口**。主仓集成不在本轮，它依赖 subject 侧路线的拍板。

批次 A（接入 + 验证）→ B（按结果定相似度下限）→ C（DeepSeek 双跑新基线）→ D（文档定稿）。A4 的主判据是 p018 / p021 / p037 三道靶子题是否翻转，而非整体指标。
...
- [ ] 遗留优化项（正式实现阶段做，非 spike 债）：~~主体识别只有字面包含一条路~~ **已实现 `findSubjects.describedAs`（nb-memory `179042c`）但未接进评测路径，效果待量化**；给 `describedAs` 定相似度下限（当前存活集非空时总会解出某个主体，需评测数据支撑）；状态检索命中率（topic 归一化 / 当前状态清单注入）。
- [ ] **把查询计划（含 `describedAs`）接进 bench 评测路径**（当前零覆盖，现为最高优先级——新能力已就位但无法验证）；接法需保留 `search()` 对照以维持可比性，建议加 `--planner heuristic|llm|off` 开关、off 为默认。
- [ ] 后续评测固定用先验知识少的答题模型（DeepSeek 实测污染率 2.1% vs mimo 16.7%，asof 计分题 4→8）。
- [ ] 补 mimo 口径的方差对照跑（两次尝试均因网关 503 中止；或直接改以 DeepSeek 为新基线重建两跑均值）。
