...
export async function prepareSystemAssets(options: {
    ...
    const writePolicy = path.resolve(systemNbookRoot) === productSystemNbookRoot && !options.productBuild
        ? "forbid" as const
        : "allow" as const;
    const llmlintSkill = writePolicy === "allow"
        ? await projectLlmlintSkill({
            sourceRoot: path.resolve(runtimePaths.applicationRoot, "packages", "llmlint", "skill"),
            targetRoot: path.resolve(systemNbookRoot, "agent", "skills", "llmlint"),
        })
        : undefined;
