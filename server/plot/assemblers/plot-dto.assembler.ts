...
export class PlotDtoAssembler {
     ...
     * 规范化 ref target。
     */
    normalizeStoryRefTarget(ref: StoryThreadRefWithTargets | StorySceneRefWithTargets): string {
     ...
     * 映射 ref DTO。
     */
    toStoryRefDto(ref: StoryThreadRefWithTargets | StorySceneRefWithTargets): StoryRefDto {
        return {
    ...
    buildEffectiveSceneRefs(scene: StorySceneWithDetails): StoryEffectiveRefDto[] {
        const threadRefs = scene.thread.refs.map((ref) => ({
            ...this.toStoryRefDto(ref),
            sourceType: "thread" as const,
            sourceId: stringifyEntityId(scene.thread.id),
        }));
        const sceneRefs = scene.refs.map((ref) => ({
            ...this.toStoryRefDto(ref),
            sourceType: "scene" as const,
            sourceId: stringifyEntityId(scene.id),
        }));

        return [...threadRefs, ...sceneRefs];
    }

    /**
     * 映射 Thread 详情 DTO。
     */
    toStoryThreadDetailDto(thread: StoryThreadWithRefs): StoryThreadDetailDto {
        return {
            ...this.toStoryThreadSummaryDto(thread),
            refs: thread.refs.map((ref) => this.toStoryRefDto(ref)),
        };
    }
