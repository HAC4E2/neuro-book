...
import {
    ...
    type Chatu8EntryClassification,
    type Chatu8InspectedEntry,
    type StoryboardImportMacroToken,
} from "nbook/shared/text-to-image-storyboard-import";
import type {
    Chatu8JsonEntrySource,
    ParsedChatu8StoryboardJson,
} from "nbook/server/text-to-image/chatu8-storyboard-json";

export const CHATU8_STORYBOARD_CHUNK_LIMITS = {
    maxEntriesPerChunk: 64,
    maxCharactersPerChunk: 80000,
} as const;

export type Chatu8StoryboardChunkSegment = {
...
export type Chatu8StoryboardChunk = {
    chunkIndex: number;
    entryCount: number;
    characterCount: number;
    segments: Chatu8StoryboardChunkSegment[];
    chunkHash: string;
};

export type Chatu8StoryboardInspection = {
    schemaVersion: "nbook.chatu8-storyboard-inspection/v1";
    sourceShape: ParsedChatu8StoryboardJson["sourceShape"];
    sourcePresetKey: string;
    rawSourceHash: string;
    sanitizedSourceHash: string;
    entries: Chatu8InspectedEntry[];
    macroTokens: StoryboardImportMacroToken[];
    chunks: Chatu8StoryboardChunk[];
...
const CLASSIFICATION_ORDER: readonly Chatu8EntryClassification[] = [
    "core_rule",
    "atomic_group",
    "scene_recipe",
    "style_quality",
...
export function inspectChatu8Storyboard(source: ParsedChatu8StoryboardJson): Chatu8StoryboardInspection {
...
function inspectEntry(sourcePresetKey: string, source: Chatu8JsonEntrySource): Chatu8InspectedEntry {
    ...
    return Chatu8InspectedEntrySchema.parse({
...
function createSourceIdentity(sourcePresetKey: string, source: Chatu8JsonEntrySource): Chatu8InspectedEntry["sourceIdentity"] {
    ...
    return {
        sourcePresetKey,
        sourceEntryId: `c8e.${createHash("sha256").update(`${identityKind}:${identityValue}`, "utf8").digest("hex").slice(0, 32)}`,
        identityKind,
...
function classifyEntry(
    ...
    const matched = new Set<Chatu8EntryClassification>();
...
function extractMacroTokens(entry: Chatu8InspectedEntry): StoryboardImportMacroToken[] {
...
function buildChunks(entries: Chatu8InspectedEntry[]): Chatu8StoryboardChunk[] {
    const chunks: Chatu8StoryboardChunk[] = [];
    let segments: Chatu8StoryboardChunkSegment[] = [];
    ...
    for (const entry of entries) {
        const sourceEntryId = entry.sourceIdentity.sourceEntryId;
        if (!entryIds.has(sourceEntryId) && entryIds.size >= CHATU8_STORYBOARD_CHUNK_LIMITS.maxEntriesPerChunk) flush();
        ...
        while (startOffset < entry.content.length) {
            if (!entryIds.has(sourceEntryId) && entryIds.size >= CHATU8_STORYBOARD_CHUNK_LIMITS.maxEntriesPerChunk) flush();
            if (characterCount >= CHATU8_STORYBOARD_CHUNK_LIMITS.maxCharactersPerChunk) flush();
            const capacity = CHATU8_STORYBOARD_CHUNK_LIMITS.maxCharactersPerChunk - characterCount;
