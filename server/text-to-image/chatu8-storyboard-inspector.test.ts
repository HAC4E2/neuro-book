...
import {
    CHATU8_STORYBOARD_CHUNK_LIMITS,
    inspectChatu8Storyboard,
} from "nbook/server/text-to-image/chatu8-storyboard-inspector";

function parse(value: object) {
    return parseChatu8StoryboardJson(new TextEncoder().encode(JSON.stringify(value)));
}

describe("deterministic Chatu8 Storyboard inspector", () => {
    it("保留 allowlist/provenance/disabled/trigger，并确定性路由七类候选与风险 flags", () => {
        const inspected = inspectChatu8Storyboard(parse({
            "示例预设": {
                entries: {
                    core: {
                        id: "director-core",
    ...
    it("fallback canonical identity 对 key 顺序稳定，但内容变化会产生新 identity", () => {
        const first = inspectChatu8Storyboard(parse({entries: [{role: "system", content: "镜头规则", enabled: true}]}));
        const reordered = inspectChatu8Storyboard(parse({entries: [{enabled: true, content: "镜头规则", role: "system"}]}));
        const changed = inspectChatu8Storyboard(parse({entries: [{role: "system", content: "另一条镜头规则", enabled: true}]}));
    ...
    it("宏区分白名单、身份、Tag fragment、随机与未知歧义", () => {
        const inspected = inspectChatu8Storyboard(parse({entries: [{
    ...
    it("chunk manifest 每块最多 64 entries / 80k 字符，且长 entry 分片不截断", () => {
        const manyEntries = Array.from({length: CHATU8_STORYBOARD_CHUNK_LIMITS.maxEntriesPerChunk + 1}, (_value, index) => ({
            id: `entry-${String(index)}`,
            role: "system",
            enabled: true,
            content: `镜头规则 ${String(index)}`,
        }));
        const many = inspectChatu8Storyboard(parse({entries: manyEntries}));
        ...
        expect(many.chunks.every((chunk) => chunk.entryCount <= CHATU8_STORYBOARD_CHUNK_LIMITS.maxEntriesPerChunk)).toBe(true);
        expect(exposedIds.size).toBe(manyEntries.length);

        const longContent = "镜头".repeat((CHATU8_STORYBOARD_CHUNK_LIMITS.maxCharactersPerChunk + 10000) / 2);
        const long = inspectChatu8Storyboard(parse({entries: [{id: "long", role: "system", enabled: true, content: longContent}]}));
        const segments = long.chunks.flatMap((chunk) => chunk.segments);
        expect(long.chunks.every((chunk) => chunk.characterCount <= CHATU8_STORYBOARD_CHUNK_LIMITS.maxCharactersPerChunk)).toBe(true);
        expect(segments.map((segment) => [segment.startOffset, segment.endOffset])).toEqual([
            [0, CHATU8_STORYBOARD_CHUNK_LIMITS.maxCharactersPerChunk],
            [CHATU8_STORYBOARD_CHUNK_LIMITS.maxCharactersPerChunk, longContent.length],
        ]);
    ...
    it("相同输入的 entries/macros/chunks 与 hash 完全稳定", () => {
        ...
        expect(inspectChatu8Storyboard(source)).toEqual(inspectChatu8Storyboard(source));
    });
});
