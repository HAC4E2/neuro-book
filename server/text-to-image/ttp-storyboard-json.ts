...
export const TTP_STORYBOARD_JSON_LIMITS = {
...
export type TtpStoryboardJsonErrorCode =
...
/** TTP Storyboard JSON 的稳定严格解析错误。 */
export class TtpStoryboardJsonError extends Error {
    readonly code: TtpStoryboardJsonErrorCode;

    constructor(code: TtpStoryboardJsonErrorCode, message: string) {
        super(`${code}: ${message}`);
        this.name = "TtpStoryboardJsonError";
        this.code = code;
    }
}

export type ParsedTtpStoryboardJson = {
    sourceShape: "direct_entries" | "wrapped_entries";
    sourcePresetKey: string;
    entries: TtpJsonEntrySource[];
...
export type ParsedStrictTtpJsonDocument = {
    sanitizedValue: TextToImageContractValue;
    sanitizedBytes: Uint8Array;
    rawSourceHash: string;
    sanitizedSourceHash: string;
...
export type TtpJsonObject = {[key: string]: TextToImageContractValue};

export type TtpJsonEntrySource = {
...
/**
 * 读取并严格解析 TTP Storyboard JSON。
 *
 * duplicate key 与 prototype key 在 token 层拒绝；原始 bytes 只用于内存 hash，返回的可持久化 bytes 已递归脱敏并 canonicalize。
 */
export function parseTtpStoryboardJson(bytes: Uint8Array): ParsedTtpStoryboardJson {
    const document = parseStrictTtpJsonDocument(bytes);
    const shape = resolveEntriesShape(document.sanitizedValue);
    if (shape.entries.length > TTP_STORYBOARD_JSON_LIMITS.maxEntries) {
        throw new TtpStoryboardJsonError(
            "STORYBOARD_IMPORT_ENTRY_LIMIT_EXCEEDED",
            `entries 超过 ${String(TTP_STORYBOARD_JSON_LIMITS.maxEntries)} 条`,
        );
    }
...
/**
 * 读取 TTP 公开导出共用的 strict JSON document 边界。
 *
 * 本函数只负责 JSON 安全、上限、secret redaction 与 canonical hash；具体 Storyboard/角色 source shape 由各自 parser 再收窄。
 */
export function parseStrictTtpJsonDocument(bytes: Uint8Array): ParsedStrictTtpJsonDocument {
    if (bytes.byteLength > TTP_STORYBOARD_JSON_LIMITS.maxBytes) {
        throw new TtpStoryboardJsonError("STORYBOARD_IMPORT_FILE_TOO_LARGE", "文件超过 16 MiB 上限");
    }
    ...
    try {
        ...
        throw new TtpStoryboardJsonError("STORYBOARD_IMPORT_UTF8_INVALID", "文件不是严格 UTF-8");
    }
...
class StrictJsonParser {
    ...
    private parseValue(depth: number): TextToImageContractValue {
        if (depth > TTP_STORYBOARD_JSON_LIMITS.maxDepth) {
            throw new TtpStoryboardJsonError("STORYBOARD_IMPORT_JSON_DEPTH_EXCEEDED", "JSON 嵌套过深");
        }
        this.skipWhitespace();
    ...
    private parseObject(depth: number): JsonObject {
        ...
        while (true) {
            ...
            if (UNSAFE_KEYS.has(key)) {
                throw new TtpStoryboardJsonError("STORYBOARD_IMPORT_JSON_UNSAFE_KEY", `禁止的 object key：${key}`);
            }
            if (keys.has(key)) {
                throw new TtpStoryboardJsonError("STORYBOARD_IMPORT_JSON_DUPLICATE_KEY", `重复 object key：${key}`);
            }
            keys.add(key);
            this.skipWhitespace();
            this.expect(":");
    ...
    private parseString(): string {
        ...
        while (this.index < this.text.length) {
            ...
            if (character === '"') {
                ...
                if (value.length > TTP_STORYBOARD_JSON_LIMITS.maxStringLength) {
                    throw new TtpStoryboardJsonError("STORYBOARD_IMPORT_JSON_STRING_TOO_LONG", "JSON 字符串超过上限");
                }
    ...
    private parseNumber(): number {
        ...
        if (Number.isInteger(value) && !Number.isSafeInteger(value)) {
            throw new TtpStoryboardJsonError("STORYBOARD_IMPORT_JSON_UNSAFE_NUMBER", "整数超出安全精度");
        }
    ...
    private invalid(message: string): never {
        throw new TtpStoryboardJsonError("STORYBOARD_IMPORT_JSON_INVALID", `${message}（offset ${String(this.index)}）`);
    }
}
...
function resolveEntriesShape(value: TextToImageContractValue): Pick<ParsedTtpStoryboardJson, "sourceShape" | "sourcePresetKey" | "entries"> {
    if (!isJsonObject(value)) {
        throw new TtpStoryboardJsonError("STORYBOARD_IMPORT_SHAPE_UNSUPPORTED", "根值必须是 object");
    }
    ...
    if (directEntries !== undefined && (Array.isArray(directEntries) || isJsonObject(directEntries))) {
        ...
        if (name.length > 500) {
            throw new TtpStoryboardJsonError("STORYBOARD_IMPORT_SHAPE_UNSUPPORTED", "source preset key 过长");
        }
        return {
    ...
    if (keys.length !== 1) {
        throw new TtpStoryboardJsonError("STORYBOARD_IMPORT_SHAPE_UNSUPPORTED", "动态 wrapper 根必须只有一个 key");
    }
    const sourcePresetKey = keys[0]!;
    const wrapped = value[sourcePresetKey];
    if (wrapped === undefined || !isJsonObject(wrapped)) {
        throw new TtpStoryboardJsonError("STORYBOARD_IMPORT_SHAPE_UNSUPPORTED", "动态 wrapper value 必须包含 entries array/map");
    }
...
function resolveEntrySources(value: TextToImageContractValue, pointer: string): TtpJsonEntrySource[] {
    if (Array.isArray(value)) {
        return value.map((entry, sourceOrder) => {
            if (!isJsonObject(entry)) {
                throw new TtpStoryboardJsonError("STORYBOARD_IMPORT_SHAPE_UNSUPPORTED", "entries 的每一项都必须是 object");
            }
    ...
    if (isJsonObject(value)) {
        return Object.keys(value).map((mapKey, sourceOrder) => {
            ...
            if (!isJsonObject(entry)) {
                throw new TtpStoryboardJsonError("STORYBOARD_IMPORT_SHAPE_UNSUPPORTED", "entries map 的每个 value 都必须是 object");
            }
            return {
                mapKey,
                sourceOrder,
    ...
    throw new TtpStoryboardJsonError("STORYBOARD_IMPORT_SHAPE_UNSUPPORTED", "entries 必须是 array 或 map");
}
