...
import type {DanbooruSourceReader} from "nbook/server/text-to-image/tag-index/danbooru-source-client";
import {
...
describe("TagIndexRuntime", () => {
    ...
    it("reads absent local status without constructing or contacting an official source client", async () => {
        const createSourceClient = vi.fn((): DanbooruSourceReader => ({
            readWatermark: vi.fn(),
            readPage: vi.fn(),
        }));
        ...
        expect(status.source.endpoint).toBe("https://danbooru.donmai.us");
