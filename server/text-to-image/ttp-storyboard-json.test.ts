...
describe("strict TTP Storyboard JSON reader", () => {
    it("支持 direct entries 与单 dynamic wrapper，并产生稳定 sanitized bytes", () => {
        const direct = parseTtpStoryboardJson(bytes(JSON.stringify({name: "direct-demo", entries: [{id: "one", content: "规则"}]})));
        const wrapped = parseTtpStoryboardJson(bytes(JSON.stringify({
            "动态预设名": {entries: [{id: "one", content: "规则"}]},
        })));
        const mapped = parseTtpStoryboardJson(bytes(JSON.stringify({
    ...
    it("token-aware 拒绝 duplicate key、prototype pollution key 与 unsafe number", () => {
        expect(() => parseTtpStoryboardJson(bytes('{"entries":[],"entries":[]}'))).toThrow(/STORYBOARD_IMPORT_JSON_DUPLICATE_KEY/u);
        for (const unsafeKey of ["__proto__", "prototype", "constructor"]) {
            expect(() => parseTtpStoryboardJson(bytes(`{"entries":[],"nested":{"${unsafeKey}":true}}`))).toThrow(
                /STORYBOARD_IMPORT_JSON_UNSAFE_KEY/u,
            );
        }
        expect(() => parseTtpStoryboardJson(bytes('{"entries":[],"value":9007199254740992}'))).toThrow(
            /STORYBOARD_IMPORT_JSON_UNSAFE_NUMBER/u,
        );
    });

    it("在任何持久化 bytes 形成前递归删除 secret，并只报告 JSON Pointer", () => {
        const first = parseTtpStoryboardJson(bytes(JSON.stringify({
            entries: [{id: "one", content: "规则", apiKey: "secret-a", nested: {authorization: "Bearer a"}}],
        })));
        const second = parseTtpStoryboardJson(bytes(JSON.stringify({
    ...
    it("canonical sanitized hash 不受对象 key 物理顺序影响，且不修改输入 bytes", () => {
        ...
        const first = parseTtpStoryboardJson(firstBytes);
        const second = parseTtpStoryboardJson(bytes('{"title":"demo","entries":[]}'));
    ...
    it("拒绝非法 UTF-8、深度/字符串/entry/文件越限", () => {
        expect(() => parseTtpStoryboardJson(new Uint8Array([0xc3, 0x28]))).toThrow(/STORYBOARD_IMPORT_UTF8_INVALID/u);

        const depth = TTP_STORYBOARD_JSON_LIMITS.maxDepth + 1;
        expect(() => parseTtpStoryboardJson(bytes(`${'{"nested":'.repeat(depth)}null${"}".repeat(depth)}`))).toThrow(
            /STORYBOARD_IMPORT_JSON_DEPTH_EXCEEDED/u,
        );
        expect(() => parseTtpStoryboardJson(bytes(JSON.stringify({
            entries: [],
            value: "x".repeat(TTP_STORYBOARD_JSON_LIMITS.maxStringLength + 1),
        })))).toThrow(/STORYBOARD_IMPORT_JSON_STRING_TOO_LONG/u);
        expect(() => parseTtpStoryboardJson(bytes(JSON.stringify({
            entries: Array.from({length: TTP_STORYBOARD_JSON_LIMITS.maxEntries + 1}, () => ({})),
        })))).toThrow(/STORYBOARD_IMPORT_ENTRY_LIMIT_EXCEEDED/u);
        expect(() => parseTtpStoryboardJson(new Uint8Array(TTP_STORYBOARD_JSON_LIMITS.maxBytes + 1))).toThrow(
    ...
    it("拒绝多 wrapper、非 object wrapper 与缺失 entries 的 unsupported shape", () => {
        for (const invalid of [
            ...
            expect(() => parseTtpStoryboardJson(bytes(JSON.stringify(invalid)))).toThrow(/STORYBOARD_IMPORT_SHAPE_UNSUPPORTED/u);
        }
    });
});
