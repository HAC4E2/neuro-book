...
describe("TTP public character visual source", () => {
    it("只投影公开明文字段，并生成稳定 character/outfit identity", () => {
        const parsed = parseTtpCharacterVisualSource(encoder.encode(JSON.stringify(source())));
        ...
        expect(parsed.characters[0]).toMatchObject({
            ...
            outfits: [{
                sourceKey: "travel",
                ownerSourceCharacterId: expect.stringMatching(/^ttp-character-[a-f0-9]{24}$/u),
                fields: {upper: "coat", lower: "trousers"},
            }],
        });
        expect(parsed.characters[0]!.sourceCharacterId).toMatch(/^ttp-character-[a-f0-9]{24}$/u);
        expect(parsed.characters[0]!.outfits[0]!.sourceOutfitId).toMatch(/^ttp-outfit-[a-f0-9]{24}$/u);
    ...
    it("object key 顺序与 images bytes 不影响 visualSourceHash，但原始文件 hash 会变化", () => {
        const first = parseTtpCharacterVisualSource(encoder.encode(JSON.stringify(source({one: "data:image/png;base64,AAAA"}))));
        const reordered = parseTtpCharacterVisualSource(encoder.encode(JSON.stringify({
    ...
    it("加密、tagData 与未知 root shape 全部 report-only，不解密或猜 schema", () => {
        const encrypted = parseTtpCharacterVisualSource(encoder.encode(JSON.stringify({
            ...source(),
            _encrypted: true,
            _nameMap: "base64",
            _version: "1.0",
        ...
        const tagData = parseTtpCharacterVisualSource(encoder.encode(JSON.stringify({...source(), tagData: {}})));
        expect(tagData.state).toBe("report_only");
        expect(tagData.diagnostics.map((item) => item.code)).toContain("TAGDATA_FORBIDDEN");

        const unknown = parseTtpCharacterVisualSource(encoder.encode(JSON.stringify({...source(), privateRuntime: {}})));
    ...
    it("未知 record 字段只隔离该记录；缺失、共享或 owner 冲突的 outfit 不产生角色 proposal", () => {
        ...
        const unknownParsed = parseTtpCharacterVisualSource(encoder.encode(JSON.stringify(unknownRecord)));
        ...
        missing.outfits = {};
        const missingParsed = parseTtpCharacterVisualSource(encoder.encode(JSON.stringify(missing)));
        ...
        const ownerParsed = parseTtpCharacterVisualSource(encoder.encode(JSON.stringify(ownerConflict)));
        ...
        const sharedParsed = parseTtpCharacterVisualSource(encoder.encode(JSON.stringify(shared)));
