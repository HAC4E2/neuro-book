...
import {
    TTP_CHARACTER_VISUAL_SOURCE_SCHEMA_VERSION,
    TtpCharacterVisualSourcePackageSchema,
    type TtpCharacterVisualSourceDiagnostic,
    type TtpCharacterVisualSourcePackage,
} from "nbook/shared/text-to-image-character-source";
import {
    TTP_STORYBOARD_JSON_LIMITS,
    parseStrictTtpJsonDocument,
...
type RawCharacter = {
    ...
    fields: Omit<TtpCharacterVisualSourcePackage["characters"][number]["fields"], "negativePrompt">;
    outfitKeys: string[];
};
...
type RawOutfit = {
    ...
    owner: string;
    names: {cn: string; en: string};
    fields: TtpCharacterVisualSourcePackage["characters"][number]["outfits"][number]["fields"];
};
...
/**
 * 解析 TTP 公开明文角色/服装 export 为不可执行 source package。
 *
 * 加密 envelope、tagData、未知 root 与无法证明 owner/ref 的记录只生成 report，不解密、不猜 schema。
 */
export function parseTtpCharacterVisualSource(bytes: Uint8Array): TtpCharacterVisualSourcePackage {
    const document = parseStrictTtpJsonDocument(bytes);
    const diagnostics: TtpCharacterVisualSourceDiagnostic[] = [];
    ...
    if (!root) {
        return packageResult(document, [], [{
            ...
            scope: "root",
            sourceKey: null,
            message: "TTP 角色视觉 export 根必须是 object",
        }]);
    }
    ...
    if (root._encrypted === true) {
        diagnostics.push({
            ...
            scope: "root",
            sourceKey: null,
            message: "加密 TTP export 只报告，不在 Route B 中解密",
        });
    }
    ...
    if (Object.keys(characterMap).length > TTP_STORYBOARD_JSON_LIMITS.maxEntries
        || Object.keys(outfitMap).length > TTP_STORYBOARD_JSON_LIMITS.maxEntries) {
        diagnostics.push({
            code: "SOURCE_SHAPE_INVALID",
            scope: "root",
            sourceKey: null,
    ...
    const characters: TtpCharacterVisualSourcePackage["characters"] = [];
    rawCharacters.forEach((character) => {
        let blocked = false;
        const sourceCharacterId = stableId("character", character.sourceKey);
        const outfits: TtpCharacterVisualSourcePackage["characters"][number]["outfits"] = [];
        character.outfitKeys.forEach((outfitKey) => {
...
function parseCharacter(
    ...
    diagnostics: TtpCharacterVisualSourceDiagnostic[],
): RawCharacter | null {
...
function packageResult(
    document: ReturnType<typeof parseStrictTtpJsonDocument>,
    characters: TtpCharacterVisualSourcePackage["characters"],
    diagnostics: TtpCharacterVisualSourceDiagnostic[],
): TtpCharacterVisualSourcePackage {
    ...
    const visualSourceHash = hashTextToImageContract({
        sourceKind: "ttp-character-export",
    ...
    return TtpCharacterVisualSourcePackageSchema.parse({
        schemaVersion: TTP_CHARACTER_VISUAL_SOURCE_SCHEMA_VERSION,
        sourceKind: "ttp-character-export",
...
function stableId(kind: "character" | "outfit", sourceKey: string): string {
    const hash = hashTextToImageContract({sourceKind: "ttp-character-export", kind, sourceKey});
    return `ttp-${kind}-${hash.slice("sha256:".length, "sha256:".length + 24)}`;
}
