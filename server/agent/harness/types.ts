...
export type InvokeAgentInput = {
    ...
    queueIfBusy?: boolean;
    /**
     * 内部 durable admission 回写：在 invocation 已写入 lifecycle start 与 durable user entry
     * （provider 推理启动前）时调用。供 character direct-write 等系统 runtime 按固定预算轮询
     * durable state；不提供时行为与默认完全一致。
     */
