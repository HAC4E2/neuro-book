...
/**
 * 正文生图角色识别子代理初始化参数。Project 归属由 session metadata 承载。
 */
export const BodyImageCharacterDetectorInitialSchema = Type.Object({}, {
    additionalProperties: false,
});

/**
 * 正文生图角色识别子代理单次输入。候选来自 Project Workspace 的 lorebook/** image-tags.md。
 */
export const BodyImageCharacterDetectorPayloadSchema = Type.Object({
    chapterPath: Type.String({description: "当前正文章节路径。为空表示调用方没有提供。"}),
    chapterMarkdown: Type.String({description: "需要识别相关角色的章节正文。"}),
    candidates: Type.Array(Type.Object({
        id: Type.String({description: "候选角色 image-tags 的稳定 id。"}),
        sourcePath: Type.String({description: "候选角色 image-tags.md 在 Project Workspace 内的路径。"}),
        cnName: Type.String({description: "角色中文名称字段，可能包含用 | 分隔的多个别名。"}),
        cnAliases: Type.Array(Type.String(), {description: "从角色中文名称拆出的中文别名。"}),
        enName: Type.String({description: "角色英文名称，用于生图 tag。"}),
    }, {additionalProperties: false}), {description: "本次可供匹配的角色 image-tags 候选。"}),
}, {additionalProperties: false});

/**
 * 正文生图角色识别子代理结构化输出。
 */
export const BodyImageCharacterDetectorOutputSchema = Type.Object({
    matches: Type.Array(Type.Object({
        id: Type.String({description: "命中的候选角色 id。"}),
        sourcePath: Type.String({description: "命中的 image-tags.md 路径。"}),
        reason: Type.String({description: "为什么该角色与正文相关。"}),
        confidence: Type.Number({minimum: 0, maximum: 1, description: "0 到 1 的置信度。"}),
    }, {additionalProperties: false}), {description: "只返回正文实际相关的角色。没有则返回空数组。"}),
    note: Type.Optional(Type.String({description: "整体说明。没有额外说明时省略。"})),
}, {additionalProperties: false});

/**
 * 正文生图插图定位子代理初始化参数。Project 归属由 session metadata 承载。
 */
export const BodyImagePromptPlacerInitialSchema = Type.Object({}, {
    additionalProperties: false,
});

/**
 * 正文生图插图定位子代理单次输入。调用方传入正文段落和已生成的 NovelAI prompt 列表。
 */
export const BodyImagePromptPlacerPayloadSchema = Type.Object({
    chapterPath: Type.String({description: "当前正文章节路径。为空表示调用方没有提供。"}),
    chapterMarkdown: Type.String({description: "当前章节完整 Markdown，只用于理解上下文，不允许改写。"}),
    paragraphs: Type.Array(Type.Object({
        id: Type.String({description: "段落稳定 id，例如 p-1。"}),
        index: Type.Number({description: "段落从 0 开始的顺序。"}),
        text: Type.String({description: "段落正文。"}),
    }, {additionalProperties: false}), {description: "调用方预切分出的可插入段落。"}),
    prompts: Type.Array(Type.Object({
        id: Type.String({description: "待插入图片 prompt 的稳定 id。"}),
        order: Type.Number({description: "prompt 在 LLM 回复中的顺序。"}),
        prompt: Type.String({description: "NovelAI 英文 tags。"}),
        responseIndex: Type.Number({description: "prompt 块在 LLM 回复中的字符位置。"}),
        nearbyText: Type.String({description: "LLM 回复中该 prompt 前方的相邻文本；没有则为空。"}),
    }, {additionalProperties: false}), {description: "正文生图 LLM 已生成的图片 prompt。"}),
    llmReply: Type.String({description: "正文生图 LLM 原始回复，供定位器理解 prompt 顺序与上下文。"}),
}, {additionalProperties: false});

/**
 * 正文生图插图定位子代理结构化输出。
 */
export const BodyImagePromptPlacerOutputSchema = Type.Object({
    placements: Type.Array(Type.Object({
        promptId: Type.String({description: "来自 prompts[].id 的图片 prompt id。"}),
        afterParagraphId: Type.String({description: "来自 paragraphs[].id；表示插入到该段落后。"}),
        reason: Type.String({description: "为什么该图适合放在该段落后。"}),
        confidence: Type.Number({minimum: 0, maximum: 1, description: "0 到 1 的置信度。"}),
    }, {additionalProperties: false}), {description: "只返回有明确位置的 prompt；无法判断位置的 prompt 不要返回。"}),
    note: Type.Optional(Type.String({description: "整体说明；没有额外说明时省略。"})),
}, {additionalProperties: false});

/**
 * 角色生图信息提取子代理初始化参数。Project 归属由 session metadata 承载。
 */
export const CharacterImageTagExtractorInitialSchema = Type.Object({}, {
    additionalProperties: false,
});

/**
 * 角色生图信息提取子代理单次输入。调用方传入当前角色详情页 Markdown。
 */
export const CharacterImageTagExtractorPayloadSchema = Type.Object({
    characterPath: Type.String({description: "当前角色详情页路径，通常是 lorebook/character/<name>/index.md。"}),
    characterTitle: Type.String({description: "当前角色显示名。"}),
    characterMarkdown: Type.String({description: "当前角色详情页 Markdown，包括 frontmatter 和正文。"}),
}, {additionalProperties: false});

/**
 * 角色生图信息提取子代理结构化输出。
 */
export const CharacterImageTagExtractorOutputSchema = Type.Object({
    cnName: Type.String({description: "角色中文名称。可直接使用角色显示名。"}),
    aliases: Type.Array(Type.String(), {description: "角色中文别名，不含空字符串。"}),
    enName: Type.String({description: "角色英文名称；无法判断时返回空字符串。"}),
    appearanceFacts: Type.String({description: "只包含生图相关的外貌、体型、年龄感、气质、服装风格和视觉特征事实。"}),
    note: Type.Optional(Type.String({description: "整体说明。没有额外说明时省略。"})),
}, {additionalProperties: false});
