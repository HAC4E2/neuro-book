...
/**
 * Leader runtime 的低代码模板副本。
 * 该文件用于模板编辑器预览与保存，不是当前生产 leader profile 的运行入口。
 */
export default function LeaderRuntimeTemplate(ctx: ProfilePromptContext<"leader.default">) {
    return (
        <ProfilePrompt>
            <HistorySet>
                <Message role="system">
                ...
                **请牢记：AI 无法取代人类的创造力。你的职责是搭建舞台和提供弹药，把最核心的创造性工作留给用户。这并不是要你抛弃创造力，你可以用创造力给用户提供灵感。**

                # System

                - Before any tool calls for a multi-step task, send a short user-visible update that acknowledges the request and states the first step. Keep it to one or two sentences.
                - 默认使用中文和用户沟通，除非用户明确要求其他语言。
                - 先理解当前 workspace 与内容节点结构，再修改文件。不要为了探索而递归枚举整个小说 workspace。
                - 普通文件读写优先使用 read_file、write_file、edit_file、apply_patch。只有需要运行仓库脚本、检查项目状态、执行验证或进行真实终端操作时，才使用 execute_shell。
                - 枚举路径时优先使用 `rg --files` 和精确路径过滤，再交给 workspace 命令解析。
                </Message>
                <Message role="system">
            ...
            <DynamicSet>
                <Message role="system">
                    {`当前线程：${String(ctx.runtime.thread.id)}`}
                </Message>
            </DynamicSet>
            <AppendingSet>
                <Reminder id="leader-runtime-focus" watchPath="scope.studio.extra">
                    <Message role="human">
                        &lt;system-reminder&gt;
                        {`\n\n【当前剧情焦点】\nThread: ${String(ctx.var("scope.studio.extra.selectedStoryThreadId") ?? "")}\nScene: ${String(ctx.var("scope.studio.extra.selectedStorySceneId") ?? "")}`}
                        &lt;/system-reminder&gt;
                    </Message>
                </Reminder>
                ...
                <Reminder id="leader-runtime-tasks" watchValue={JSON.stringify(ctx.scope.agent.tasks ?? null)} repeatEveryTurns={5}>
                    <Message role="human">
                        &lt;system-reminder&gt;
                        {`\n\n【当前任务状态】\n${JSON.stringify(ctx.scope.agent.tasks ?? null)}`}
                        &lt;/system-reminder&gt;
                    </Message>
                </Reminder>
                <Reminder
                    id="workspace"
                    when={Boolean(ctx.scope.studio.workspace)}
                    watchPath="scope.studio.workspace"
                    repeatEveryTurns={20}
                >
                    <Message role="system">
                        &lt;system-reminder&gt;
                        {`Agent 默认 workspace 为容器目录：workspace/。当前小说目录为：${ctx.scope.studio.workspace ?? ""}/。常规任务以当前小说目录为边界；文件工具优先在当前小说目录查找裸 lorebook/...、manuscript/... 与相对路径。workspace/... 表示容器级路径，不要用它跨小说、跨 session 或跨 thread 探索，除非用户明确要求。`}
                        &lt;/system-reminder&gt;
                    </Message>
                </Reminder>
                <Watch
                ...
                />
                <If condition={Boolean(ctx.runtime.options.planModeReminder)}>
                    <Reminder id="plan-mode">
                        <Message role="system">
                            {String(ctx.runtime.options.planModeReminder ?? "")}
                        </Message>
                    </Reminder>
                </If>
                <Message role="human">
                ...
                <Message role="human" source="input">
                    {"prompt" in ctx.input ? ctx.input.prompt : ""}
                </Message>
            </AppendingSet>
        </ProfilePrompt>
    );
