...
import type {AgentStreamEvent, ProfileInputMap, RunOptions, SubAgentCompletionResult, SubAgentProfileKey} from "nbook/server/agent/types";
...
export class SubAgentThread<TKey extends SubAgentProfileKey> extends BaseAgentThread<TKey> {
     ...
     * 派发一次新的 subagent 任务。
     */
    async dispatchRun(input: ProfileInputMap[TKey], options?: RunOptions): Promise<void> {
     ...
     */
    async *invokeStream(input: ProfileInputMap[TKey], options?: RunOptions): AsyncIterable<AgentStreamEvent> {
     ...
     */
    async run(input: ProfileInputMap[TKey], options?: RunOptions): Promise<SubAgentCompletionResult<TKey>> {
