...
describe("Workbench Chrome", () => {
    ...
    it("keeps the desktop activity bar focused on project tools and moves global navigation into the title bar", () => {
        ...
        expect(browserWorkspace.primary.find((item) => item.id === "text-to-image")).toEqual({
            id: "text-to-image",
            disabled: false,
        });

        const userAssetsWorkspace = createWorkbenchActivityItems({
            desktopAvailable: false,
            surfaceActive: true,
            userAssetsMode: true,
        });
        expect(userAssetsWorkspace.primary.find((item) => item.id === "text-to-image")).toEqual({
            id: "text-to-image",
            disabled: true,
        });
    });
