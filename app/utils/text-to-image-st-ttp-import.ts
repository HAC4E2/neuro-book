...
export type StTtpTextToImageImportResult = {
...
export function parseStTtpTextToImageSettings(data: unknown): StTtpTextToImageImportResult {
...
function unwrapSettingsRoot(data: unknown): JsonRecord {
    ...
    // 以下 key 是上游导出格式（智绘姬 / st-chatu8）的真实字段名，需保持原样才能解析外部文件
...
function readPromptRule(record: JsonRecord): TextToImagePromptReplacementRule | null {
    ...
    return {
        id: readFirstString(record, ["id"]) || `st-ttp-rule-${hashString(`${trigger}\n${replacement}`)}`,
