        ...
        <template #header>
            ...
            <div class="flex min-w-0 flex-1 items-center justify-between gap-4">
                <div class="flex min-w-0 items-center gap-4">
                    ...
                    <div class="min-w-0">
                        <h2 class="truncate text-xl font-bold text-[var(--text-main)]">{{ draft?.title || "规则档案" }}</h2>
        ...
        <div v-if="draft" class="space-y-3 text-xs" :class="savingFile ? 'pointer-events-none opacity-80' : ''">
            ...
            <section class="rounded-xl border border-rose-500/15 bg-rose-500/5 p-3">
                <div class="grid grid-cols-[minmax(0,1fr)_132px_150px] gap-2">
                    <label class="space-y-1"><span>规则名</span><input v-model="draft.title" class="field" @blur="void saveDraft()"></label>
                    ...
                    <label class="space-y-1"><span>分类</span><input :value="draft.subtype ?? ''" class="field" list="rule-subtypes" @input="draft.subtype = ($event.target as HTMLInputElement).value || null" @blur="void saveDraft()"></label>
                </div>
                ...
                <label class="mt-2 block space-y-1"><span>标签</span><TagInput :model-value="draft.tags" placeholder="添加标签..." accentStyle @update:model-value="draft.tags = $event; void saveDraft()" /></label>
            </section>
            ...
            <section class="grid grid-cols-[1fr_1fr] gap-3">
                <div class="space-y-2 rounded-xl border border-[var(--border-color)] bg-[var(--bg-input)]/40 p-3">
                    <div class="font-semibold text-[var(--text-main)]">触发与效果</div>
                    <input :value="draft.rule.scope ?? ''" class="field" placeholder="适用范围" @input="draft.rule.scope = ($event.target as HTMLInputElement).value || null" @blur="void saveDraft()">
                    <input :value="draft.rule.priority ?? ''" class="field" placeholder="优先级 / 权重" @input="draft.rule.priority = ($event.target as HTMLInputElement).value || null" @blur="void saveDraft()">
                    <textarea :value="draft.rule.trigger ?? ''" rows="3" class="textarea" placeholder="触发条件" @input="draft.rule.trigger = ($event.target as HTMLTextAreaElement).value || null" @blur="void saveDraft()"></textarea>
                    <textarea :value="draft.rule.effect ?? ''" rows="4" class="textarea" placeholder="规则效果" @input="draft.rule.effect = ($event.target as HTMLTextAreaElement).value || null" @blur="void saveDraft()"></textarea>
                </div>
                <div class="space-y-2 rounded-xl border border-[var(--border-color)] bg-[var(--bg-input)]/40 p-3">
                    <div class="font-semibold text-[var(--text-main)]">边界与冲突</div>
                    <textarea :value="draft.rule.limits ?? ''" rows="3" class="textarea" placeholder="限制与边界" @input="draft.rule.limits = ($event.target as HTMLTextAreaElement).value || null" @blur="void saveDraft()"></textarea>
                    <TagInput :model-value="draft.rule.exceptions" placeholder="例外..." @update:model-value="draft.rule.exceptions = $event; void saveDraft()" />
                    <TagInput :model-value="draft.rule.conflicts" placeholder="冲突规则..." @update:model-value="draft.rule.conflicts = $event; void saveDraft()" />
                    <TagInput :model-value="draft.rule.examples" placeholder="例子..." @update:model-value="draft.rule.examples = $event; void saveDraft()" />
                </div>
            </section>
