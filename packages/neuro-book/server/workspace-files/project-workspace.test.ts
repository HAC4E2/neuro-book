...
describe("initProjectDatabaseAtRoot", () => {
    it("新建 Project SQLite 会包含文生图最终 Prompt Bundle 列", async () => {
        const projectRoot = await fs.mkdtemp(path.join(tmpdir(), "nbook-project-schema-"));
        try {
    ...
    it("旧 Project SQLite 缺少最终 Prompt Bundle 列时会幂等补齐且保留已有资产", async () => {
        const projectRoot = await fs.mkdtemp(path.join(tmpdir(), "nbook-project-asset-migration-"));
