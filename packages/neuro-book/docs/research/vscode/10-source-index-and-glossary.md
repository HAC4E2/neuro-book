...
| local extension host | 在用户本地运行的扩展宿主 | 桌面可能是 Node process，Web 场景可能是 WebWorker |
...
| registry epoch/version | 描述/内存视图每次原子发布后的版本水位 | 用来判断快照新旧，不是业务 revision 全替代 |
| durable job | 有持久记录、状态、恢复和结果/回流语义的后台任务 | 不保证外部 side effect 可安全重放 |
| authority | 某类状态或副作用的唯一允许写入/读取守门人 | UI preview 不等于 authority commit |
