...
| 模块 | 当前状态 | 依据 |
| --- | --- | --- |
| 写作模式 v1 | 主路径阶段完成，进入体验打磨 | [Task 64](.agents/tasks/64-world-engine-prompt-engineering/README.md)、[Task 87](.agents/tasks/87-plot-two-trees-and-writer-modes/README.md)、[Task 124](.agents/tasks/124-writing-pipeline-batch3/README.md) |
| World Engine | 核心模型、API、Workbench 和作者主路径阶段完成 | [Task 56](.agents/tasks/56-world-engine/README.md)、[Task 65](.agents/tasks/65-world-engine-calendar-enhancement/README.md)、[Task 71](.agents/tasks/71-world-engine-codeact-readwrite/README.md) |
| Plot | 两棵树模型已落地：承载树负责章节呈现，因果树负责剧情组织，`StoryScene` 连接两者 | [Task 78](.agents/tasks/78-plot-scene-world-engine-bridge/README.md)、[Task 93](.agents/tasks/93-plot-planning-layer/README.md)、[Task 99](.agents/tasks/99-plot-planning-ui/README.md) |
| Agent / Workflow | 主要链路已实现；Provider API / Automatic Model Discovery 已在 PR #101 合并并完成 Task 104 收尾，真实 Project、外部 Provider 和完整浏览器产品流程仍待做 | [Task 104](.agents/tasks/104-pi-models-runtime-upgrade/README.md)、[Task 111](.agents/tasks/111-workflow-agent-integration/README.md)、[Task 116](.agents/tasks/116-agent-workflow-reliability/README.md)、[Task 139](.agents/tasks/139-agent-abort-error-projection/README.md) |
...
- [Task 139](.agents/tasks/139-agent-abort-error-projection/README.md) 将主动取消与运行错误分开：取消显示中性状态，保留已生成的半截正文，并避免重复错误气泡。
- [Task 138](.agents/tasks/138-agent-conversation-branch-projection/README.md) 将对话分支切换改为基于可见对话锚点的投影，运行期记账 entry 不再制造假分支。
- [Task 111](.agents/tasks/111-workflow-agent-integration/README.md) 已补齐 Workflow 的持久身份、公开投影、Job/Run 详情、`wf.ask` 和 Composer/Preview 防重复提交；动态 `outputSchema` 的 `report_result` 合同也已补齐。
...
- 测试写入 Project Workspace 的高风险路径已切换到隔离 Runtime Workspace Root；相关清理竞态和真实根残留已有专项记录，详见 [Task 125 Round 04](.agents/tasks/125-runtime-artifact-storage-lifecycle/walkthroughs/round-04-workspace-test-isolation.md)。
