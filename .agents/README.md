...
- `issues/`：重大/长期目标的未授权远端草稿与Draft-Key迁移规则；取得编号后按真实结果创建唯一`planned` Task，闭合链接与授权留痕后最后删除草稿。
- `tasks/ownership.json`：稳定 Task 名到根/主应用物理 owner 的唯一索引；是否关联Issue由工作用途决定，解析失败不得跨 root fallback。
- `tasks/`：Task schema、人机分工、任务产物、修改计划、完成门禁、Leader继续条件、Design密封diff、双根任务记录和正式证据索引。
