...
- `agentWorkflow`由Leader填写。`kind: design`必须有非空的`设计类型`、`设计产物`、`决策范围`、`允许文件`章节；API类型必须路由`api-and-interface-design`。Design Tasker可与开发者直接协作，只更新允许文件中的Proposal/Spec草案与报告；普通Tasker不修改Issue、Proposal、Spec、Task范围或owner。
