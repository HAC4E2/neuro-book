...
- `worktreeId`、`branchId`：实际执行身份；治理文档直接在当前工作区修改时可为`null`。
...
- `profile`固定为`nbook.agent-skills/v1`；`kind`允许`feedback`、`research`、`design`、`bug`、`feature`、`refactor`、`docs`、`release`、`migration`。
- `kind: research`只产出本 Task 的研究证据、开发者观察、决策简报和决定记录；不修改 Proposal、Spec 或业务源码。`kind: design`只产出Task指定的唯一Proposal/Spec草案、证据和决策简报；两者都可由Tasker直接与开发者协作。
...
`kind: research`的README必须包含四个非空章节：

- `## 研究问题`：允许研究和请求开发者决定的人话问题；
- `## 研究产物`：本 Task `walkthroughs/`或`evidences/`中的精确文件路径；
- `## 决策范围`：开发者可在本 Task 内确认的取舍，以及不进入本 Task 的决定；
- `## 允许文件`：只列本 Task 的walkthrough/evidence精确路径，并包含全部研究产物。

Research Task不得写 Proposal、Spec、业务源码或其它 Task；README与context仍由Leader维护。研究决定只在Task报告中形成durable record，Leader再据此推进后续单产物design Task。
...
4. 大目标可先建`research` Task；research Tasker直接与开发者完成证据、观察和决定记录，但不写Proposal/Spec。长期方案或合同目标明确后，Leader再建唯一产物的`design` Task。
5. Leader检查ownership和编号，创建Task README、context、切片、owner和agentWorkflow；默认顺序执行。
6. 普通Tasker只实现、验证和报告；research Tasker只修改Task列出的研究报告与证据；design Tasker只修改Task列出的唯一Proposal/Spec与报告文件。
...
历史兼容使用明确截止线，不以“缺字段”自动判定历史：根Task的`00149`与两个`00150`保留无`agentWorkflow`旧画像，`00152+`必须使用完整当前合同；应用Task ownership迁移基线截至`148`，`149+`必须使用完整当前合同。`00158-notification-contrast-fix`只豁免历史缺失`context.md`，其它字段仍校验。旧`docs/tasks/`迁移文件保持原编号和正文，不为流程切换批量改写。
