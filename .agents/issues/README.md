...
3. GitHub返回正整数编号后，Leader把该编号写入全部扁平 Task 的 `actionIssueId`，再建立 draft/planned Task 合同。
4. 确认远端 Issue、Proposal、Spec和Task链接闭合后，删除本地草稿；Leader walkthrough记录草稿路径、Issue编号和迁移结果。不得保留第二份状态正文或兼容入口。
