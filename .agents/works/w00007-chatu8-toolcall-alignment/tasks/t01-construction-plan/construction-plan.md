...
日期：2026-09-08。状态：施工已完成；实现提交 `0ac96723` 已应用到主工作区，旧同步 merge 已结案，并已完成 HAC4E2 master 合并和 push。
...
本文已按阶段 A–E 执行。默认值、配置、请求解码、五类调用方、设置 UI、导入导出和离线验收均已完成，并已应用到主工作区；HAC4E2 `origin/master=106f5e7b` 已解决冲突并合并为 `4faa1205`，已 push 到 `new-text-to-picture`。真实 Provider/Model、浏览器人工验收、PR、发布和部署未执行，实际命令和退出码记录在 t02 Task 的验证证据中。
