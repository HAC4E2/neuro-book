...
- Windows 运行包不再整棵复制开发依赖和文档生成物：核心运行文件从约 3 万个降到 4700 个，体积从约 1.2 GB 降到 154 MB。解压、扫描和后续桌面封装需要处理的文件都更少。

### 修复

- 切换书籍时会先退出当前编辑工作面，等目标书籍真正打开后才加载配置、文件树和写作工具。以前切换得太快时，页面可能先请求尚未打开的书籍并显示“项目未打开”。
- 旧版升级中被误标为“需要确认”的三份 AI 会话会自动修复，原来的 Project 归属和真实最后活动时间会恢复；真正无法确定归属的会话可以在项目选择页重新绑定，或明确改为不绑定某本书。
...
- 使用 NeuroBook Manager 安装、更新或启动时，会先检查升级计划，再自动备份并升级应用数据库、图片附件和 AI 会话；只有新版本通过启动与版本检查后才确认升级，失败会停止本次新进程并恢复旧数据和旧版本。
- 如果你直接运行源码或解压后的 Product，没有经过 Manager，先执行 `bun run migrate:application-state -- --plan` 查看计划，再执行 `bun run migrate:application-state -- --apply`。迁移中断时不要重新开始，按输出的 runId 选择 `--resume` 或 `--rollback`。
