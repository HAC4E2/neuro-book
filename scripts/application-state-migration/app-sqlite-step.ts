...
} from "nbook/scripts/db/app-sqlite-migrations";
import {
    optionalFileHash,
    pathExists,
    renameDurable,
...
export async function applyAppSqliteMigrationStep(options: AppSqliteStepOptions): Promise<AppSqliteStepResult> {
    ...
    if (manifest) {
        assertDatabaseIdentity(manifest, plan.databasePath);
        ...
        if (manifest.state === "applied") {
            if (plan.pendingMigrationIds.some((id) => manifest.pendingMigrationIds.includes(id))) {
                throw new Error(`App SQLite migration run ${options.runId} 标记完成但 schema 仍有 pending。`);
            }
            return {appliedMigrationIds: manifest.appliedMigrationIds};
        }
    ...
    if (remaining.pendingMigrationIds.some((id) => manifest.pendingMigrationIds.includes(id))) {
        throw new Error(`App SQLite migration run ${options.runId} apply 后仍有本轮 pending migration。`);
    }
    const targetHash = await requiredFileHash(manifest.databasePath);
    manifest = {
        ...manifest,
        state: "applied",
        appliedMigrationIds: [...manifest.pendingMigrationIds],
        targetHash,
    };
    await writeAtomicDurableJson(path, manifest);
    return {
        appliedMigrationIds: manifest.appliedMigrationIds.length > 0
            ? manifest.appliedMigrationIds
