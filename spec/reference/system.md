...
`bun assets/agent/scripts/workspace.ts validate --root WORKSPACE_ROOT` 至少检查：
...
1. 调用脚手架，例如 `bun assets/agent/scripts/workspace.ts new lorebook/character/苏雪 --root WORKSPACE_ROOT --dir --type character --title 苏雪`。
2. 读取生成的 `index.md` 模板。
3. 编辑 frontmatter 与正文。
4. 运行 `bun assets/agent/scripts/workspace.ts validate --root WORKSPACE_ROOT lorebook/character/苏雪`。
